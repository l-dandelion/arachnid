...
var DefaultOSAttibutes = map[int]OSAttributes{
...
func init() {
	for browser, userAgentData := range Database {
		...
		osAttributes := DefaultOSAttibutes[os]
		for version, _ := range userAgentData.Formats {
			ua := createFromDetails(
				browser,
				version,
				osAttributes.OSName,
				osAttributes.OSVersion,
				osAttributes.Comments,
			)
	...
	idx := r.Intn(l)
	UserAgents["all"][0], UserAgents["all"][idx] = UserAgents["all"][idx], UserAgents["all"][1]
	UserAgents["common"][0], UserAgents["common"][idx] = UserAgents["common"][idx], UserAgents["common"][1]
}
...
func createFromDetails(bname, bver, osname, osver string, c []string) string {
	if bver == "" {
		bver = Database[bname].TopVersion
	}
